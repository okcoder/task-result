<template>
  <el-row class="priority_type">
    <el-col :span="4" :key="priorityType" :class="priorityType" v-for="(schedules,priorityType) in priorityTypes">
      ...
      <draggable :options="options" group="priorityTypes" :list="schedules">
        <el-card class="box-card" v-for="schedule in schedules" :key="schedule.id" shadow="hover">
          <div slot="header" class="clearfix" @click="onSelectedSchedule(schedule)">
            ...
            <el-button style="float: right; padding: 3px 0" @click="onEditSchedule(schedule.id)" type="text">edit
            </el-button>
          </div>
        </el-card>
      </draggable>
    ...
    <el-col :span="6">
      ...
      <el-card class="box-card" shadow="hover" v-show="schedule.id !== undefined">
        ...
        <div>estimatePrepareTime: {{Math.floor(schedule.estimatePrepareTime/60)}}:{{schedule.estimatePrepareTime%60}}
        </div>
